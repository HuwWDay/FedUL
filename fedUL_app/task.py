...
# Replace argparse with a simple config class for Flower
class Config:
    percent = 0.1
    lr = 1e-4
    wdecay = 0.0
...
def prepare_data():
    ...
    if args.classnum == 2:
        client_train_data, client_test_data, prior_test, client_priors_corr, client_Pi = \
            data_utils.MNIST_SET(data_path="./data", clientnum=args.clientnum, setnum_perclient=args.setnum)
        ...
        client_train_data, client_validation_data, client_test_data, prior_test, client_priors_corr, client_Pi = \
            data_utils.MNIST_SET_Multiclass(
                data_path="./data", clientnum=args.clientnum, setnum_perclient=args.setnum, noniid=args.noniid)
    ...
    for i in range(len(client_train_data)):
        ...
        train_loaders.append(torch.utils.data.DataLoader(train_set, batch_size=args.batch, shuffle=True, drop_last=True))
        validation_loaders.append(torch.utils.data.DataLoader(val_set, batch_size=args.batch * 5, shuffle=False))
        test_loaders.append(torch.utils.data.DataLoader(test_set, batch_size=args.batch * 5, shuffle=False))
...
def train(model, train_loader, optimizer, loss_fun, device, Pi, priors_corr, prior_test):
    model.train()
    loss_all = 0
